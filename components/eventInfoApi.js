...
router.get('/events/:uid', async (req, res) => {
    try {
        ...
        let [rows] = await connection.query('SELECT event_id, event_name, event_datetime, location, description, max_guests_count, current_guests_count, invited_guests_count, event_visibility, creator_uid FROM events WHERE creator_uid = ?', [uid]);
...
router.get('/events/joined/:uid', async (req, res) => {
    try {
        let uid = req.params.uid;
        let [rows] = await connection.query(`
            SELECT e.event_id, e.event_name, e.creator_uid, e.event_datetime, e.location, e.event_visibility, u.username AS creator_username
            FROM events AS e
            JOIN users AS u ON e.creator_uid = u.uid
...
router.get('/public/events', async (req,res) => {
    try {
        let [rows] = await connection.query(`
            SELECT e.event_id, e.event_name, e.location, e.event_datetime, e.current_guests_count, e.max_guests_count, e.description, e.creator_uid, u.username AS creator_username
            FROM events AS e
            JOIN users AS u ON e.creator_uid = u.uid
