...
async function run() {
  ...
  try {
    ...
    const connections = await ConnectToDatabase('Localhost', 'app', 'fiberline', 'app');
    const [rows, fields] = await connections.execute('SELECT url, titleSelector, dateSelector, name FROM eventsites');//nicht vergessen
    ...
    if (rows.length > 0) {
      ...
      for (const row of rows) {
        ...
        // Call ConnectToDatabase with the required parameters
        const connection =await ConnectToDatabase('Localhost', 'app', 'fiberline', 'app');
